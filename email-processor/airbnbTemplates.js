/**
 * Guest reply templates for the Airbnb chat-relay auto-reply system.
 *
 * This is a CommonJS port of whatsapp-bot/src/templates.js's {id, matchWhen,
 * reply} shape - kept as a SEPARATE file, not shared, because whatsapp-bot is
 * an ESM project and this one is CommonJS. A template change meant to apply
 * to both channels needs applying to both files. See the plan for this
 * tradeoff (spicy-wobbling-dragonfly.md).
 *
 * Each entry:
 *   id          — stable key used everywhere else in the code (never shown to the guest)
 *   matchWhen   — plain-English description given to Claude for intent matching
...
const TEMPLATES = [
  ...
  },
  {
    ...
    // Deliberately no reply (disabled 2026-09-09 at the owner's request):
    // Airbnb already sends its own automated welcome, so proposing ours too
    // duplicated it. The id is KEPT so a bare "hi" still classifies as a
    // greeting and results in no action - deleting it would let greetings fall
    // through to casual_ack ("You're most welcome!") or an unmatched staff
    // alert on every hello.
    reply: null,
  },
  {
...
Please take a photo of your QR ticket as a backup — in case the physical ticket is lost, you can still scan the photo to enter or exit the carpark.`,
  ...
  {
    id: 'late_checkout',
    // Kept narrow on purpose (same wording as whatsapp-bot/src/templates.js):
    // "how to check out tomorrow" used to match here and get the fee policy.
    matchWhen:
...
To confirm luggage storage, please reply "Yes". Once confirmed, we will send you the storage details and information. 😊 Thank you for your understanding!`,
  },
  {
    ...
    // an auto-reply (same as UNMATCHED). See airbnbChatReply.js.
    reply: null,
  },
  {
    id: 'smoking',
...
/** Topics that always warrant a staff room-alert alongside any canned reply - see airbnbChatReply.js. */
const TECHNICAL_ISSUE_TOPICS = new Set([
  'ac_not_working',
  'water_heater',
  'stove',
  'door_lock_instructions',
  'qr_not_working',
]);
